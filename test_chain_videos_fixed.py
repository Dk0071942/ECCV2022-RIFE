...
def save_interpolated_frames_as_video(frames_dir, output_path, fps=25):
    ...
    if first_frame is None:
        raise Exception(f"Could not read first frame: {frames[0]}")
    
    height, width = first_frame.shape[:2]
    
    # Create video using pattern that matches our 7-digit format
    ffmpeg_cmd = [
        'ffmpeg', '-y',
        '-r', str(fps),
        '-i', str(frames_dir / 'frame_%07d.png'),  # Changed from %05d to %07d
        '-s', f'{width}x{height}',
        '-c:v', 'libx264',
        '-preset', 'veryfast',
        '-crf', '18',
    ...
    if not success:
        # If it fails, try listing the actual pattern
        print(f"FFmpeg failed with pattern frame_%07d.png")
        print(f"First few frames in directory:")
        for i, f in enumerate(sorted(frames_dir.glob("*.png"))[:5]):
            print(f"  {f.name}")
...
def test_chain_videos():
    ...
    try:
        ...
        transition1_path, status1 = interpolator.interpolate(
            ...
            num_passes=2,
            fps=25,
            use_disk_based=False  # Try without disk-based first
        )
        ...
        # Also test the actual ChainedInterpolator
        print("\n🧪 Testing ChainedInterpolator class...")
        from rife_app.services.chained import ChainedInterpolator
        
        chained = ChainedInterpolator(model)
        chained_output, chained_status = chained.interpolate(
            anchor_video_path=str(video1_path),
            middle_video_path=str(video2_path),
            end_video_path=str(video1_path),
            num_passes=2,
            final_fps=25,
            use_disk_based=False
        )
        
        print(f"\n✅ ChainedInterpolator test successful!")
        print(f"   Output: {chained_output}")
        print(f"   Status: {chained_status}")
        
        # Copy the chained output
        shutil.copy2(chained_output, output_dir / f"chained_class_output_{timestamp}.mp4")
